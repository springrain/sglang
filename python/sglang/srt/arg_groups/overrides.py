...
def _deepseek_v4_sm120_moe(view: Any) -> dict:
    """KTransformers: pin an unset/"auto" speculative MoE runner backend to the
    resolved target one. The draft (nextn) layers hold the same MXFP4-packed
    expert tensors, and a speculative backend left at "auto" resolves to the
    fp4-unaware triton fallback in Fp8Config.get_quant_method, which crashes
    the draft forward with a hidden-size mismatch.

    The SM120 MXFP4 default itself moved to the model-override registry
    (arg_groups/model_overrides/deepseek_v4.py, gated on fp4 experts); this
    slot pass keeps the speculative-backend pin at its legacy slot, where
    server_args.py invokes it by name."""
    hf_config = model_config_of(view).hf_config
    if hf_config.architectures[0] != "DeepseekV4ForCausalLM":
        return {}
    if (
        view.moe_runner_backend != "auto"
        and view.speculative_algorithm is not None
        and view.speculative_moe_runner_backend in (None, "auto")
    ):
        logger.info(
            f"Use {view.moe_runner_backend} as speculative MoE runner backend for "
...
def _a2a_fusion_adjustments(view: Any) -> dict:
    ...
    fusion enabled; FlashInfer, FlashInfer MegaMoE, and DeepEP v2 A2A require
    it disabled.
    """
    # KTransformers: expert offload (--kt-weight-path) requires shared experts
    # to stay on the GPU; it is incompatible with Waterfill fusion.
