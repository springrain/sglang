...
def run_resolution_pipeline(server_args: Any) -> None:
    ...
    # Normalize downstream compatibility inputs before CUDA-graph parsing.
    # This intentionally precedes the dummy-model boundary: a parsed config
    # should expose the same aliases even when no model is materialized.
    from sglang.srt.arg_groups.kt_hook import (
        enforce_kt_cuda_graph_compatibility,
        handle_kt_compatibility,
    )

    run_hook(handle_kt_compatibility, server_args)
    ...
    # KT's host-side expert path is a hard capture restriction, including for
    # explicitly requested graph backends, so it is the final graph decision.
    run_hook(enforce_kt_cuda_graph_compatibility, server_args)
