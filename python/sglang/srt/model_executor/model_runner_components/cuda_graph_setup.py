...
def _register_kt_capture_token_buckets(model_runner: "ModelRunner") -> None:
    ...
    # The resolved per-phase config lives in the exec.graph namespace bag
    # (see runtime_context); ``server_args.cuda_graph_config`` stays the raw
    # user input and may be None when the per-phase convenience flags were
    # used instead, so read the resolved config here.
    prefill_buckets = getattr(
        getattr(get_exec().graph, "cuda_graph_config", None), "prefill", None
    )
