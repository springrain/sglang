...
class ServerArgs:
    ...
    def resolve_once(self) -> None:
        ...
        try:
            # ktransformers: normalize KT compatibility aliases before CUDA
            ...
            # overrides whatever the hooks decided, like the legacy in-place
            # pass that ran right after _handle_cuda_graph_config).
    ...
    # -------------------------------------------------------------------------
    # ktransformers (KT) resolution helpers. These are the forward-ported KT
    # passes the legacy monolithic __post_init__ ran inline; the hook-pipeline
    # architecture invokes them from resolve_once / check_server_args instead.
    # -------------------------------------------------------------------------
    ...
    def _enforce_kt_cuda_graph_compatibility(self) -> None:
        ...
        Adapted to the hook-pipeline architecture: this runs after
        ``run_resolution_pipeline`` and reads the resolved
        ``cuda_graph_config``; the restriction is recorded through
        ``declare_resolution`` so the published config bags and the
        ``/server_info`` projection carry it (field writes alone would not:
        the bags are projected from the declaration stash).
        """

        if not (
            self.kt_weight_path or self.kt_lora_path or self.kt_expert_lora_path
        ):
            return

        cuda_graph_config = resolving_view(self).cuda_graph_config
        if cuda_graph_config is None or not isinstance(
            cuda_graph_config, CudaGraphConfig
        ):
            # The pipeline returned before CUDA-graph resolution (e.g. the
            # dummy-model short circuit); nothing captured to restrict. The
            # legacy disable_cuda_graph switch was already set by
            # _handle_kt_args where applicable.
            return
        ...
        if self.kt_weight_path and (
            ...
            new_config = with_phase(
                new_config, Phase.PREFILL, backend=Backend.DISABLED
            )
        ...
        if self.kt_lora_path or self.kt_expert_lora_path:
            ...
            if new_config.decode.backend != Backend.DISABLED:
                new_config = with_phase(
                    new_config, Phase.DECODE, backend=Backend.DISABLED
                )
        ...
        if new_config is not cuda_graph_config:
            declared_fields["cuda_graph_config"] = new_config
        if declared_fields:
            declare_resolution(
                self, "_enforce_kt_cuda_graph_compatibility", **declared_fields
            )
    ...
    def check_server_args(self):
        ...
        # ktransformers: validate the extended KT expert placement and CPU
        # runtime options (reads the raw KT fields, independent of the hook
        # validation families).
