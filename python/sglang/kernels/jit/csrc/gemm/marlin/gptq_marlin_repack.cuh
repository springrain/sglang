...
__global__ void gptq_marlin_repack_kernel(
  ...
  if constexpr (input_n_major) {
    // One grid-y slice owns one expert. Native and Marlin layouts contain the
    // same number of packed words, so this offset is valid for both buffers.
  ...
  auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id) {
    ...
    if constexpr (has_perm) {
      ...
      if (threadIdx.x < stage_size) {
        ...
        if constexpr (input_n_major) {
          // MXFP4 checkpoints store packed weights as [N, K / 8] int32.
          // Populate the K-major shared tile without a full tensor transpose.
          int4 values;
...
#define CALL_IF_N_MAJOR_REPACK(NUM_BITS)                                                                              \
    ...
    host::RuntimeDeviceCheck(cudaFuncSetAttribute(                                                                    \
        ...
        cudaFuncAttributeMaxDynamicSharedMemorySize,                                                                  \
        max_shared_mem));                                                                                             \
    host::LaunchKernel(dim3(blocks, num_experts), device::marlin::repack_threads, stream,                             \
                       static_cast<std::size_t>(max_shared_mem))(                                                    \
        device::marlin::gptq_marlin_repack_kernel<device::marlin::repack_threads, NUM_BITS, false, true>,             \
...
// Repack native [N, K / 2] MXFP4 bytes into caller-owned Marlin storage.
// Batched input uses grid.y to process every expert in a single launch.
