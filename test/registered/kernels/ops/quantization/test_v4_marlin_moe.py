...
@pytest.mark.parametrize(
    "activation_kwargs",
    [
        pytest.param({}, id="silu"),
        pytest.param(
            {"activation": "swiglu_oai", "swiglu_alpha": 1.702},
            id="swiglu-oai",
        ),
        pytest.param(
            {
...
def test_v4_streamed_ready_group_matches_filtered_full_layer(activation_kwargs):
    from sglang.srt.layers.quantization.v4_marlin_moe import (
        allocate_v4_mxfp4_marlin,
        apply_v4_marlin_moe,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param({}, id="silu"),
        pytest.param({"swiglu_limit": 0.0}, id="silu-zero-limit"),
        pytest.param({"swiglu_limit": 2.0}, id="silu-clamp"),
        pytest.param(
            {
        ...
        pytest.param(
            ...
            id="situ-no-linear-softcap",
        ),
    ],
)
def test_v4_marlin_moe_reference_determinism_and_graph(activation_kwargs):
    from sglang.srt.layers.quantization.v4_marlin_moe import (
        apply_v4_marlin_moe,
        prepare_v4_mxfp4_marlin,
    )
    ...
    for token in range(m):
        for slot in range(topk):
            ...
            activation = activation_kwargs.get("activation", "silu")
            if activation == "situ":
                beta = activation_kwargs["situ_beta"]
                linear_beta = activation_kwargs["situ_linear_beta"]
                gate = beta * torch.tanh(gate / beta) * torch.sigmoid(gate)
                if linear_beta > 0.0:
                    up = linear_beta * torch.tanh(up / linear_beta)
                activated = (gate * up).to(torch.bfloat16)
            else:
                swiglu_limit = activation_kwargs.get("swiglu_limit")
                if swiglu_limit is not None and swiglu_limit > 0.0:
                    gate = gate.clamp(max=swiglu_limit)
                    up = up.clamp(min=-swiglu_limit, max=swiglu_limit)
                if activation == "swiglu_oai":
                    alpha = activation_kwargs["swiglu_alpha"]
                    activated = (
                        gate * torch.sigmoid(alpha * gate) * (up + 1.0)
                    ).to(torch.bfloat16)
                else:
